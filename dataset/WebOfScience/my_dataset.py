...
if not os.path.exists(os.path.join(base_path, "few-shot")):
    os.mkdir(os.path.join(base_path, "few-shot"))
...
def few_shot_sample(dataset, shot=5, seed=550, type="train"):
    ...
    for idx, pair in enumerate(train):
        examples.append(InputExample(guid=str(idx), text_a=str(idx), label=pair[1]))
...
def sub_dataset(shot=4, seed=550, ratio=-1, ratio_flag=0):
